  ...
  def get(self, request):
    ...
    try:
      ...
      for user_details in user_details_obj:
        career_item_obj = CareerItems.objects.filter(career__user=user_details.user, key__in=[CareerItemKey.LANGUAGE.value, CareerItemKey.FRAMEWORK.value, CareerItemKey.DATA_BASE.value]).order_by('key').values_list('value', flat=True)
        joining_date_time = datetime.combine(user_details.joining_date, time())
        affiliation_period = Utils.get_elapsed_period(joining_date_time, date_now)
        results.append({
...
class CareerDicRetrieveAPIView(RetrieveAPIView):
  ...
  def get(self, request):
    ...
    try:
      career_obj = Careers.objects.filter(user__company=request.user.company, user=user_id)
      career_item_obj = CareerItems.objects.filter(career__user = user_id)
...
Returns:
  _type_: _description_
"""
class AggregateCareerAPIView(APIView):
  authentication_classes = [JWTAuthentication]
  permission_classes = []
  ...
  def get(self, request):
    ...
    try:
      career_obj = Careers.objects.filter(user__company=request.user.company, user__status=settings.USER_EFFECTIVE_STATUS)
      ...
      for career in career_obj:
        if(career.start_date is None or career.end_date is None):
          continue
        career_item_obj = CareerItems.objects.filter(career_id=career.id)
        period_Month = Utils.get_elapsed_period(Utils.sub_day(career.start_date, 1), career.end_date)
        db_dic = ViewUtil.getCareerItemPointDic(CareerItemKey.DATA_BASE.value, db_dic, career, career_item_obj, int(period_Month[1]))
        lang_dic = ViewUtil.getCareerItemPointDic(CareerItemKey.LANGUAGE.value, lang_dic, career, career_item_obj, int(period_Month[1]))
        framework_dic = ViewUtil.getCareerItemPointDic(CareerItemKey.FRAMEWORK.value, framework_dic, career, career_item_obj, int(period_Month[1]))

      db_dic_sorted = sorted(db_dic.items(), key=lambda x:x[1], reverse=True)
      lang_dic_sorted = sorted(lang_dic.items(), key=lambda x:x[1], reverse=True)
      framework_dic_sorted = sorted(framework_dic.items(), key=lambda x:x[1], reverse=True)

      result = {
        'careerDic': {
          CareerItemKey.DATA_BASE.value: dict((x, y) for x, y in db_dic_sorted),
          CareerItemKey.LANGUAGE.value: dict((x, y) for x, y in lang_dic_sorted),
          CareerItemKey.FRAMEWORK.value: dict((x, y) for x, y in framework_dic_sorted),
        }
      }
      ...
      print('【ERROR】:' + traceback.format_exc())
      error_message = 'スキル情報集計処理中にエラーが発生しました。'
      if type(e) == exceptions.ValidationError:
        error_message = e.detail
      response = Response()
      response.status_code = status.HTTP_400_BAD_REQUEST
...
class CareerListAPIView(ListAPIView):
  ...
  def get(self, request):
    ...
    try:
      ...
      total_count = Careers.objects.filter(**where_params).count()
      career_obj = Careers.objects.filter(**where_params).order_by("start_date")[int(ofset):(int(ofset) + int(limit))]
...
class CareerRetrieveAPIView(RetrieveAPIView):
  ...
  def get(self, request):
    ...
    try:
      career_obj = Careers.objects.get(id=career_id, user__company_id=request.user.company.id, user_id=request.user.id)

      dic = {}
      for key_name in CareerItemKey.KEY_LIST.value:
        dic[key_name] = CareerItems.objects.filter(key=key_name, career_id=career_id).values_list('value', flat=True)
...
class SaveCareerAPIView(CreateAPIView):
  ...
  def create(self, request):
    ...
    if not 'projectName' in request.data:
      raise exceptions.APIException('Invalid Parameter:projectName')
    ...
    try:
      with transaction.atomic():
        ...
        if datetime.strptime(f'{start_date} 00:00:00', '%Y/%m/%d %H:%M:%S') > datetime.strptime(f'{end_date} 00:00:00', '%Y/%m/%d %H:%M:%S'):
          raise exceptions.ValidationError('開始日、終了日の大小関係が不正です。')
        ...
        career_req = {
          ...
          'start_date': start_date.replace('/', '-') if start_date and start_date != '' else None,
          'end_date': end_date.replace('/', '-') if end_date and end_date != '' else None,
        }

        if career_id:
          career_obj = Careers.objects.select_for_update().get(id = career_id)
        ...
        for item_key in CareerItemKey.KEY_LIST.value:
          ...
          if career_id:
            # DBに存在している値
            career_item_obj = CareerItems.objects.select_for_update().filter(career_id=career_id, key=item_key).values_list('value', flat=True)
            ...
            for val in del_values:
              del_data = CareerItems.objects.filter(career_id=career_id, key=item_key, value=val)
              del_data.delete()
          else:
            add_values = career_item
...
class CareerDestroyAPIView(DestroyAPIView):
  ...
  def destroy(self, request):
    ...
    try:
      career_obj = Careers.objects.get(id = career_id)
      career_obj.delete()
...
class CareerOutputAPIView(APIView):
  ...
  def post(self, request):
    param_user_id = request.data['userId']
    ...
    try:
      ...
      career_obj = Careers.objects.filter(**where_params).order_by("start_date")
      ...
      for career in career_obj:
        dic = {}
        for key_name in CareerItemKey.KEY_LIST.value:
          values = list(CareerItems.objects.filter(key=key_name, career_id=career.id).values_list('value', flat=True))
          # カンマ区切りで結合
...
"""
class ViewUtil():
  def getCareerItemPointDic(key_name, dic, career_obj, career_item_obj, months):
  ...
  def getCareerItemDic(key_name, dic, career_obj, career_item_obj, months):
    item = filter(lambda item: item.career.id == career_obj.id and item.key == key_name, career_item_obj)
    for i in item:
      db_dic_key = i.value
      if db_dic_key in dic.keys():
        dic[db_dic_key] = dic[db_dic_key] + months
      else:
        dic[db_dic_key] = months
    return dic
