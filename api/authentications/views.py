...
class NormalAuthentication(BaseAuthentication):
  def authenticate(self, request):
    ...
    try:
      user_obj = Users.objects.filter(user_id=user_id, status=settings.USER_EFFECTIVE_STATUS).first()
      if user_obj is None:
...
class JWTAuthentication(BaseAuthentication):
  ...
  def authenticate(self, request):
    ...
    try:
      ...
      try:
        user = Users.objects.get(id=user_id, status=settings.USER_EFFECTIVE_STATUS)
